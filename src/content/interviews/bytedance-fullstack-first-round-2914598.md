...
summary: "字节跳动中国交易与广告全栈一面面经。从最近在学什么切入，先问 Vue 3 Composition API 的工作原理与相比旧写法的优势，再手撕带并发限制的异步调度器 Scheduler（含 add 方法返回 Promise），并追问为什么把 while 改成 if、六个任务跑完的时间与打印顺序，最后深挖 SSE 断线重连、TaskID/MessageID 分配、流式半 JSON 解析与首屏优化。"
---
...
1. 自我介绍。
2. 聊一下你最近在学什么东西？
3. 简历里提到用过 Vue 3 的 Composition API，能简单解释一下它是怎么工作的吗？
4. 没有它的时候是怎么写的？有了它带来什么优势？
5. 算法：JS 实现一个带并发限制的异步调度器 Scheduler，保证同时运行的任务最多有两个，完善代码中的 Scheduler 类，使给定程序能正确输出。
6. 新增 add 功能：`Scheduler.add` 返回一个 Promise，当添加的任务被执行时，它返回的 Promise 被 resolve；自己再补一组对返回值的测试。
7. 我看到有两个地方，第一个是为什么把 `while` 改成了 `if`？出于什么考虑？会带来哪些变化？
8. 现在是 A、B、C、D 打印，打到 D 的时候一共走了多少毫秒？（答：600ms）
9. 那这样 A 和 D 都是 600ms，它们两个同时结束谁先打印？（答：A 先打印，A 的 setTimeout 在 0ms 就创建好了，D 的 setTimeout 在 500ms 才创建。）
10. 你项目里有自己解决问题的经验吗？印象比较深刻的那种。（讲了 SSE 断线重连）
11. 你刚刚讲的 TaskID 和 MessageID 是谁分配的？
12. 前端本地会把那个 ID 缓存下来，缓存在哪里？（状态管理、localStorage）
13. 除了这个还有吗？（讲了前端对接大模型 ToolCall、处理流式半 JSON 解析、项目打包首屏加载优化等）
14. ToolCall 返回的问题具体是怎么解决的？这是个比较常见的问题，大家都很容易遇到。有调研哪些社区的方案？最后怎么决策、怎么解决的？讲一下细节。
